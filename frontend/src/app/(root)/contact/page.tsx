...
function ContactForm() {
  ...
  const mutation = useMutation<any, Error, FormData>({
    mutationFn: (newContact: FormData) => {
      return axiosInstance.post('/contact-us', newContact)
    },
  })
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      />
      <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-full transition-colors duration-300" disabled={mutation.isPending}>
        <Send className="mr-2 h-5 w-5" /> {mutation.isPending ? 'Sending...' : 'Send Message'}
      </Button>
      {mutation.isError && (
        <div className="text-red-500">An error occurred: {mutation.error.message}</div>
      )}
      {mutation.isSuccess && (
        <div className="text-green-500">Thank you for your message. We'll get back to you soon!</div>
      )}
    </form>
  )
}
